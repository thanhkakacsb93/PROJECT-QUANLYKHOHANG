...
const Login = () => {
    const { stateloginUser, setstateloginUser, stateloginAdm, setstateloginAdm, NavigateExportCommand, NavigateListRepo } = useContext(StorContext)
        ...
        axios.get("https://64c481e967cfdca3b660cf14.mockapi.io/api/dataUsers")
            .then((rs) => {
                if (rs.data.some((item) => item.Username === values.Username &&
                    ...
                    if (NavigateExportCommand === 1) {
                        navigate("/ExportCommand")
                    }
                    if (NavigateListRepo === 1) {
                        navigate("/list1")
                    }
                    else {
                        navigate('/');
